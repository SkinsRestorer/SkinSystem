...
if( basename(__FILE__) == basename($_SERVER["SCRIPT_FILENAME"]) ) {
    ...
    if(( basename(__FILE__) == basename($_SERVER["SCRIPT_FILENAME"]) ) && grabGetValue('user') !== false) {
        ...
        if ($_GET['dl'] == 'true') {header("Content-Disposition: attachment; filename=".grabGetValue('user').".png");}
        if(grabGetValue('format') == 'raw'){
            header("Content-Type: image/png");
            echo file_get_contents($player->getSkinFile());
            // header('Content-Type: text/plain');
            // echo $player->getSkinFile();
        } else {
        ...
        public function getSkinFile() {
            ...
            if ($now - filemtime($cacheDir.'.lastCleanup') >= (60*60)) {
                ...
                foreach (glob($cacheDir.'*') as $file) {
                    if (is_file($file) && ($now - filemtime($file) >= $seconds_to_cache)) {unlink($file);}
                }
            }
            return $skinFile;
        }
