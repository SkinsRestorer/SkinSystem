...
function printErrorAndDie($data = [])
{
    ...
    if (isset($data['footer'])) {
        $data['footer'] = '<div class="container">' . $data['footer'] . '</div>';
    }
...
function cacheGrab($url, $nm, $dirhead = '', $max_cache = false, $hchk = false)
{
    ...
    } // no max cache? touch so it's not caught by cleanup
...
function cacheClean($dirhead = '')
{ // initiate cleanup only every 15 mins or so
    global $config;
    $cdir = $dirhead . $config['cache_dir'];
    if (!is_dir($cdir)) {
        mkdir($cdir, 0775, true);
    ...
    if ($now - filemtime($cdir . 'index.php') >= (15 * 60)) {
        ...
        foreach (glob($cdir . '*') as $cl) {
            if (is_file($cl) && ($now - filemtime($cl) >= $config['cache_for_days'] * 24 * 60 * 60)) {
                unlink($cl);
            }
        }
    }
