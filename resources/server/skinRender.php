...
if ((basename(__FILE__) == basename($_SERVER["SCRIPT_FILENAME"])) && ((grabGetValue('user') !== false) || (grabGetValue('mojang') !== false))) {
    ...
    if (grabGetValue('format') == 'raw') {
        echo file_get_contents($skinRaw);
    } else { // cache system for "rendered" skins
        unset($renderparams[0]);
        unset($renderparams[1]);
        $cdir = __DIR__ . '/../../' . $config['cache_dir'];
        mkdir($cdir, 0775, true);
        $cfile = $cdir . $skinHash . '-' . hash("md5", serialize($renderparams));
        if (!is_file($cfile) or file_get_contents($cfile) == '') {
            $player->get3DRender($skinRaw, $cfile);
        }
...
class render3DPlayer
{
    ...
    public function getSkinFile()
    {
        ...
        if (strlen($skinURL) < 1) {
            $skinURL = $this->fallback_img;
        } // if can't get skin, use steve.
        if (!$mj256) {
            preg_match('/[^\/]+$/', $skinURL, $mj256);
        }
